...
export default function DashboardCalendarBlock({ role }: { role: "mjpm" | "deliverer" }) {
  ...
  useEffect(() => {
    load();
    // Optionnel : polling auto-refresh
  ...
  if (err) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-red-700 flex items-center justify-between">
        <span>{err}</span>
        <button className="btn-secondary" onClick={load}>Réessayer</button>
      </div>
    );
  }

  return <MissionsCalendar missions={missions} beneficiaries={beneficiaries} role={role} />;
}
